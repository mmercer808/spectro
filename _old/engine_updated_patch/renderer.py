...
class Renderer:
    ...
    def execute_to_target(self, cmdlist: CommandList, target: RenderTarget):
        # Default: render to all attachments (color + optional pick)
        target.fbo.use()
        ...
        for cmd in cmdlist.commands:
            if isinstance(cmd, CmdSetViewport):
                ...
                # Pick-only is kept for compatibility, but the preferred path is MRT (lit shader writes pick too).
                pick_only = (cmd.pipeline_id == "pick_id")

                if pick_only and target.pick is not None:
                    target.fbo.use(attachments=[1])
                else:
                    target.fbo.use()

                prog = self.registry.get_pipeline(cmd.pipeline_id).program
                vao = mesh.vao_pick if pick_only else mesh.vao_lit

                # Auto-provide u_id_rgba when the shader supports it (MRT picking)
