...
- [x] Ensure all engine modules are importable (display demo runs)
- [x] Fix any circular import issues (TimeCamera deferred in manager.py)
- [ ] Create `__init__.py` files if missing

### Display-only demo (run first, no audio)
- **Run:** `python demo/run_demo.py` from project root
- Shows: SyncRegion (transport bar), WaveformRegion (placeholder), LanesRegion (timeline + lanes + playhead)
- No AudioEngine, no buffers_v2; use to verify layout before Phase 2
...
dispatcher.set_loop_length(beats)  # Collection solver: length
dispatcher.set_beats_per_bar(n)   # brlength
# Quad stream over any sample: one timeline (internal or from any audio file) → four streams (live MIDI, sequencer, waveform, +1). Formal spec (Timeline–Solver–Dispatcher–Event Manager) and running over an analyzed sample (e.g. heavenfalls.mp3): docs/timeline_solver_dispatcher_spec.md. See also docs/dispatcher_equation.md § Quad stream over any sample.
```
